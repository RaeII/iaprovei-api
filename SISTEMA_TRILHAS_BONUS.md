...
Ajusta dificuldade baseada na performance da última parada e performance geral:
...
selectDifficulty(performance: StudyTrailPerformance, lastStopPerformance?: number): DifficultyLevel {
  ...
  // Boa performance na última parada (≥80%) → aumenta dificuldade
  if (lastStopPerformance && lastStopPerformance >= 80) {
    if (performance.success_rate >= 75) return DifficultyLevel.HARD;
    if (performance.success_rate >= 60) return DifficultyLevel.MEDIUM;
    return DifficultyLevel.EASY;
  }

  // Performance ruim na última parada (<60%) → mantém dificuldade
  if (lastStopPerformance && lastStopPerformance < 60) {
    if (performance.success_rate >= 70) return DifficultyLevel.MEDIUM;
    return DifficultyLevel.EASY;
  }

  // Performance média → ajusta baseado na performance geral
  if (performance.success_rate >= 80) return DifficultyLevel.HARD;
  if (performance.success_rate >= 65) return DifficultyLevel.MEDIUM;
  return DifficultyLevel.EASY;
}
```
...
- **Adaptativa** (padrão): Ajusta baseada na performance da última parada
